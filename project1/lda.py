...
if __name__ == "__main__":
    ...
    train_x_pca = pca.fit_transform(train3_x)
    test_x_pca = pca.transform(test3_x)

    LDA = LinearDiscriminantAnalysis(n_components=2)
    LDA.fit(train_x_pca,train3_y)
    train_x_lda = LDA.transform(train_x_pca)
    test_x_lda = LDA.transform(test_x_pca)
    n_classes = 3  #总类数
    n_features = test_x_lda.shape[1] 
    ...
    for i in range(test3_num):
        p = Gauss_predict(test_x_lda[i,:],mu,var)
        if p+1 == test3_y[i]:
            C+=1
    score = C/test3_num
    print(score)
